...
class SimCLR(BaseMethod):
    def __init__(self, backbone=None, projection_head=None, criterion=NTXentLoss(), mode="train", **aug_args):
        # super().__init__()
    ...
    def linear_predict(self, x, linear_mode=None):
        if self.mode != "linear_eval":
            raise RuntimeError("Linear evaluation mode is not enabled.")
        self.backbone.eval()
        self.classifier.train() if linear_mode == "train" else self.classifier.eval()
    ...
    def finetune_predict(self, x, linear_mode=None):
        if self.mode != "finetune":
            raise RuntimeError("Finetune mode is not enabled.")
        self.backbone.eval()
        self.classifier.train() if linear_mode == "train" else self.classifier.eval()
        with torch.no_grad():  # Disable gradient calculation during prediction.
            # This code is just test for the UT-HAR dataset
    ...
    def _configure_optimizer(self):
        ...
        if self.mode == "linear_eval":
            ...
            params = [
                ...
                {"params": self.classifier.parameters(), "lr": 0.06},
            ]
            self.optimizer = torch.optim.Adam(params, weight_decay=1e-5)
        else:
            params = [
