...
class MMFi_Dataset(Dataset):
    def __init__(self, data_base, data_unit, modality, split, data_form, action_labels):
            ...
            for action in actions:
                if self.data_unit == 'sequence':
                    data_dict = {'modality': self.modality,
                                 ...
                                 'subject': subject,
                                 'action': action,
                                 'gt_path': os.path.join(self.data_base.data_root,
    ...
    def __getitem__(self, idx):
        ...
        if self.data_unit == 'sequence':
            ...
            raise ValueError('Unsupport data unit!')
        return sample, self.action_labels


def make_dataset(dataset_root, config):
    database = MMFi_Database(dataset_root)
    config_dataset, action_labels = decode_config(config)
...
def collate_fn_padd_wifi_only(batch_and_labels):
    '''
    Padds batch of variable length
    '''
    batch = [i[0] for i in batch_and_labels]
    action_labels = batch_and_labels[0][1]
    ...
    labels = [action_labels.index(a) for a in batch_data['action']]
    wifi_batch_data = batch_data['input_wifi-csi'].permute(0, 2, 3, 4, 1)
    N, A, S, _, _ = wifi_batch_data.size()
    wifi_batch_data = wifi_batch_data.contiguous().view(N, A, S, -1)
    return wifi_batch_data, labels


def collate_fn_padd_for_mmfi(batch):
    '''
    Padds batch of variable length
    '''
...
def make_dataloader(dataset, is_training, generator, batch_size, collate_fn_padd=collate_fn_padd_wifi_only):
    loader = DataLoader(
        dataset,
