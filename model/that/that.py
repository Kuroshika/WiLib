...
class HARTrans(torch.nn.Module):
    def __init__(self, args):
        ...
        self.transformer = HARTransformer(args.waves, args.hlayers, args.hheads, args.packets)
        self.args = args
        self.kernel_num = args.kernel_num
        self.kernel_num_v = args.kernel_num_v
        self.filter_sizes = args.filter_sizes
        self.filter_sizes_v = args.filter_sizes_v
        #
        self.packets = args.packets
        self.antennas = args.antennas
        self.subcarriers_per_antenna = args.subcarriers_per_antenna

        # self.pos_encoding = Gaussian_Position(90, 500, args.K)
        self.pos_encoding = Gaussian_Position(args.waves, args.packets, args.K)
        if args.vlayers == 0:
            self.v_transformer = None
            self.dense = torch.nn.Linear(args.waves, 7)
        else:
            self.v_transformer = Transformer(self.packets, args.vlayers, args.vheads)
            self.dense = torch.nn.Linear(
                self.kernel_num * len(self.filter_sizes) + self.kernel_num_v * len(self.filter_sizes_v), 7)
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            ...
            self.__setattr__(enc_attr_name,
                             torch.nn.Conv1d(in_channels=args.waves,
        ...
        for i, filter_size in enumerate(self.filter_sizes_v):
            ...
            self.__setattr__(enc_attr_name_v,
                             torch.nn.Conv1d(in_channels=self.packets,
    ...
    def forward(self, data):
        N, _, T, S = data.size()
        x = data.squeeze()
        x = x.view(N, -1, self.args.sample, S)
        ...
        if self.v_transformer is not None:
            y = data.view(-1, self.packets, self.antennas, self.subcarriers_per_antenna)
...
class TransCNN(torch.nn.Module):
    def __init__(self, args):
        ...
        self.softmax = torch.nn.LogSoftmax(dim=1)
        self.transformer = Transformer(args.waves, args.hlayers, 9)
        self.args = args
        self.kernel_num = args.kernel_num
        self.kernel_num_v = args.kernel_num_v
        self.filter_sizes = args.filter_sizes
...
class TransformerM(torch.nn.Module):
    def __init__(self, args):
        ...
        self.cls = torch.nn.Parameter(torch.zeros([1, 1, args.waves], dtype=torch.float, requires_grad=True))
        self.sep = torch.nn.Parameter(torch.zeros([1, 1, args.waves], dtype=torch.float, requires_grad=True))
