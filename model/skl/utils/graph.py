import numpy as np
import torch
from model.skl.utils import tools
...
class Graph:
    ...
    def __init__(self,
        ...
        assert layout in ['openpose', 'nturgb+d', 'coco', 'handmp', 'antenna_graph','antenna_graph_gat']
...
inward_ori_index = [(1, 2), (2, 3), (3, 1)]
inward = [(i - 1, j - 1) for (i, j) in inward_ori_index]
outward = [(j, i) for (i, j) in inward]
neighbor = inward + outward
class AdjMatrixGraph:
    def __init__(self, K=3, sample_size=5,num_node=3):
        ...
        self.A_sep = tools.seperated_adjacency(self.A_binary_with_I, [0,1,2])