...
import math
import numpy as np
from model.skl.utils.graph import AdjMatrixGraph
...
class STAttentionBlock(nn.Module):
    def __init__(self, in_channels, out_channels, inter_channels, num_subset=8, num_point=25, num_frame=32,
        ...
        if use_spatial_att:
            atts = torch.zeros((1, num_subset, num_point * self.window_size, num_point))
            ...
            if glo_reg_s:
                self.attention0s = nn.Parameter(
                    torch.ones(1, num_subset, num_point * self.window_size, num_point) / num_point,
                    requires_grad=True)
        ...
        if use_temporal_att:
            ...
            self.pet = PositionalEncoding(out_channels, num_point, num_frame, 'temporal')
