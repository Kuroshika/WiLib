...
from utils.base_util import load_unsupervised_data_n_model
import argparse
...
def main():
    ...
    parser.add_argument(
        "--model",
        choices=["MLP", "LeNet", "ResNet18", "ResNet50", "ResNet101", "RNN", "GRU", "LSTM", "BiLSTM", "CNN+GRU", "ViT"],
    )
    ...
    root = "./Data/"
    unsupervised_train_loader, supervised_train_loader, test_dataloader, model = load_unsupervised_data_n_model(
        args.model, root
    )
    ...
    for epoch in range(100):
        ...
        print(
            f"epoch [{epoch + 1}/{100}], total loss:{total_loss:.4f},kl loss:{kl_loss:.4f},eh loss:{eh_loss:.4f},he loss:{he_loss:.4f},kde loss:{kde_loss:.4f}"  # noqa: E501
        )
    #######################################
    ...
    def test():
        ...
        print(f"Test accuracy: {100 * correct_1 / total:.2f}%, {100 * correct_2 / total:.2f}%")
