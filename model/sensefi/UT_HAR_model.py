...
class UT_HAR_MLP(nn.Module):
    def __init__(self,input_T,input_S):
        super(UT_HAR_MLP, self).__init__()
        self.input_T = input_T
        self.input_S = input_S
        self.fc = nn.Sequential(
            nn.Linear(self.input_S * self.input_T, 1024),
            nn.ReLU(),
            nn.Linear(1024, 128),
            nn.ReLU(),
            # nn.Linear(128, 7)
    ...
    def forward(self, x):
        x = x.view(-1, self.input_T * self.input_S )
        # x = self.fc(x)
        return x
...
class UT_HAR_LSTM(nn.Module):
    def __init__(self,
        ...
        self.lstm = nn.LSTM(self.input_S, self.hidden_dim, num_layers=num_layers)
        # self.fc = nn.Linear(hidden_dim, 7)
...
class UT_HAR_BiLSTM(nn.Module):
    def __init__(self,
        ...
        self.input_S = input_S
        self.hidden_dim = hidden_dim
        self.lstm = nn.LSTM(self.input_S, self.hidden_dim, num_layers=num_layers, bidirectional=True)
        # self.fc = nn.Linear(hidden_dim, 7)
...
class UT_HAR_CNN_GRU(nn.Module):
    def __init__(self,
        ...
        self.encoder = nn.Sequential(
            # input size: (250,90)
            nn.Conv1d(self.input_T, self.input_T, 12, 3),
            nn.ReLU(True),
            nn.Conv1d(self.input_T, self.input_T, 5, 2),
            nn.ReLU(True),
            nn.Conv1d(self.input_T, self.input_T, 5, 1)
            # 250 x 8
        )
        self.gru = nn.GRU(gru_input_size, gru_hidden_size, num_layers=1)
